...
Your pipeline now builds, tests, scans, and deploys Docker images automatically. But a single host running `docker compose` has limits — no automatic failover, no horizontal scaling, no zero-downtime deployments. Next: [Orchestration](/technical/docker-in-depth/09-orchestration) — how Docker Swarm and Kubernetes manage containers at scale.
